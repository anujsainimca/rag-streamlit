...
def generate_response(documents, api_key, query):
...
if uploaded_file is not None and openai_api_key and query:
    ...
    response = generate_response(documents, openai_api_key, query)
    if response:
        st.write("Response:")
        st.write(response)
    else:
